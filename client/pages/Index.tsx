...
function AnalyzeView({ selectedAccent, setSelectedAccent, fileName, setFileName, runAnalysis, analysisStatus, transcript, setTranscript }: { selectedAccent: string; setSelectedAccent: (value: string) => void; fileName: string; setFileName: (value: string) => void; runAnalysis: (details: { transcript: string; accent: string; language: string }) => void; analysisStatus: string; transcript: string; setTranscript: (value: string) => void }) {
  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
      <section className="relative overflow-hidden rounded-[24px] border border-[#63e6e9]/25 bg-[linear-gradient(145deg,#132546,#10182e_65%,#211b40)] p-5 shadow-[0_20px_60px_rgba(7,20,48,0.24)] sm:p-6"><div className="pointer-events-none absolute -right-16 -top-16 h-40 w-40 rounded-full bg-[#63e6e9]/10 blur-3xl" /><div className="relative z-10 mb-6 flex items-start justify-between"><div><div className="mb-2 flex items-center gap-2"><span className="rounded-full border border-[#63e6e9]/20 bg-[#63e6e9]/10 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.14em] text-[#9ee7e9]">Step 01</span><span className="text-[9px] text-[#7185aa]">Audio source</span></div><h2 className="text-[18px] font-semibold tracking-[-0.03em] text-[#f4f7ff]">Bring in your voice</h2><p className="mt-1 text-[11px] text-[#8fa1c0]">Upload a file or capture a live sample.</p></div><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#63e6e9]/20 to-[#a78bfa]/20 text-[#63e6e9]"><FileAudio size={18} /></div></div><input ref={inputRef} type="file" accept="audio/*,.wav,.mp3,.m4a" className="hidden" onChange={(event) => { const selectedFile = event.target.files?.[0]; if (!selectedFile) return; const validType = selectedFile.type.startsWith("audio/") || AUDIO_EXTENSIONS.test(selectedFile.name); if (!validType) { toast.error("Choose a supported audio file: WAV, MP3, M4A, AAC, OGG, FLAC, or WebM."); event.currentTarget.value = ""; return; } if (selectedFile.size > MAX_AUDIO_SIZE) { toast.error("Audio files must be smaller than 25 MB."); event.currentTarget.value = ""; return; } setAudioUrl(URL.createObjectURL(selectedFile)); setFileName(selectedFile.name); toast.success(`${selectedFile.name} added to this evaluation.`); }} /><button onClick={() => inputRef.current?.click()} className="group relative flex w-full flex-col items-center justify-center overflow-hidden rounded-2xl border border-dashed border-[#5d78a6] bg-[#0e1a32]/80 px-5 py-10 transition hover:border-[#63e6e9]/70 hover:bg-[#122241]"><div className="mb-3 flex h-11 w-11 items-center justify-center rounded-xl bg-[#63e6e9]/10 text-[#63e6e9] transition group-hover:scale-105"><UploadCloud size={20} /></div><p className="text-[12px] font-semibold text-[#dce7dd]">Drop an audio file here</p><p className="mt-1 text-[10px] text-[#758184]">or click to browse from your device</p></button><div className="my-5 flex items-center gap-3 text-[9px] uppercase tracking-[0.15em] text-[#7185a7]"><span className="h-px flex-1 bg-white/[0.09]" />quick start<span className="h-px flex-1 bg-white/[0.09]" /></div><div className="grid grid-cols-2 gap-2"><button onClick={handleRecordClick} className={`flex items-center justify-center gap-2 rounded-lg border px-3 py-2.5 text-[10px] font-semibold transition ${recording ? "recording-pulse border-[#fb7185]/40 bg-[#fb7185]/10 text-[#ffc1c5]" : "border-white/[0.1] text-[#c2cdc4] hover:border-white/[0.2]"}`}>{recording ? <CircleStop size={14} /> : <Mic size={14} />}{recording ? `Stop · 00:${String(recordingSeconds).padStart(2, "0")}` : "Record audio"}</button><button onClick={() => setShowDemos(!showDemos)} className="flex items-center justify-center gap-2 rounded-lg border border-white/[0.1] px-3 py-2.5 text-[10px] font-semibold text-[#c2cdc4] transition hover:border-white/[0.2]"><Library size={14} />Demo samples<ChevronDown size={12} className={`transition ${showDemos ? "rotate-180" : ""}`} /></button></div>{showDemos && <div className="mt-3 space-y-2 rounded-xl border border-white/[0.07] bg-[#1b2644] p-2">{demoSamples.map((sample) => <button key={sample.name} onClick={() => { setFileName(`${sample.accent.toLowerCase().replace(/ /g, "-")}-demo.wav`); setShowDemos(false); toast.success(`${sample.accent} demo loaded.`); }} className="flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left transition hover:bg-white/[0.05]"><span className="flex h-7 w-7 items-center justify-center rounded-md bg-[#a78bfa]/10 text-[#a78bfa]"><Play size={11} fill="currentColor" /></span><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-semibold text-[#dce6de]">{sample.name}</span><span className="text-[9px] text-[#748083]">{sample.accent} · {sample.duration}</span></span><ArrowRight size={12} className="text-[#657276]" /></button>)}</div>}{fileName && <div className="mt-5"><AudioPlayer fileName={fileName} src={audioUrl || undefined} onPlay={() => undefined} />{transcript && <div className="mt-3 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-[#63e6e9]/15 bg-[#63e6e9]/[0.04] p-3"><div className="mb-1 flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.14em] text-[#63e6e9]"><Activity size={11} />Detected transcript</div><p className="text-[11px] leading-5 text-[#c5d6d1]">{transcript}</p></div><div className="rounded-xl border border-[#a78bfa]/15 bg-[#a78bfa]/[0.04] p-3"><div className="mb-1 flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.14em] text-[#c4b5fd]"><ArrowRight size={11} />Translated transcript</div><p className="text-[11px] leading-5 text-[#d8d0f2]">{translateTranscript(transcript, targetLanguage)}</p><p className="mt-2 text-[9px] text-[#8990ae]">{languages.find((item) => item.value === targetLanguage)?.label || "Target language"}</p></div></div>}{transcript && <div className="mt-3 rounded-xl border border-[#67e8f9]/15 bg-[#67e8f9]/[0.04] p-3"><div className="mb-1 flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.14em] text-[#67e8f9]"><Activity size={11} />Live transcript</div><p className="text-[11px] leading-5 text-[#c5d6d1]">{transcript}</p></div>}{recordingError && <p className="mt-2 text-[10px] text-[#ffc1c5]">{recordingError}</p>}</div>}</section>
      <section className="rounded-[24px] border border-[#a78bfa]/20 bg-[linear-gradient(145deg,#1a2140,#15172f)] p-5 shadow-[0_20px_60px_rgba(7,20,48,0.2)] sm:p-6"><div className="mb-6 flex items-start justify-between"><div><div className="mb-2 flex items-center gap-2"><span className="rounded-full border border-[#a78bfa]/25 bg-[#a78bfa]/10 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.14em] text-[#c4b5fd]">Step 02</span><span className="text-[9px] text-[#8290b3]">Context & language</span></div><h2 className="text-[18px] font-semibold tracking-[-0.03em] text-[#f4f7ff]">Tell us who is speaking</h2><p className="mt-1 text-[11px] text-[#8fa1c0]">Language detection and accent labels improve the comparison.</p></div><button onClick={() => toast.info("Accent labels power the per-group WER and CER comparison.")} className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#a78bfa]/10 text-[#c4b5fd] hover:bg-[#a78bfa]/20"><Info size={16} /></button></div><label className="mb-2 block text-[10px] font-bold uppercase tracking-[0.14em] text-[#748083]">Spoken language</label><div className="relative mb-2"><select value={language} onChange={(event) => setLanguage(event.target.value)} className="w-full appearance-none rounded-lg border border-white/[0.09] bg-[#1b2644] px-3 py-2.5 text-[11px] text-[#dce6de] outline-none focus:border-[#63e6e9]/50">{[{ value: "auto", label: "Auto-detect from speech" }, ...languages.filter((item) => item.value !== "auto")].map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><ChevronDown size={14} className="pointer-events-none absolute right-3 top-3 text-[#718083]" /></div><div className="mb-3 flex items-center justify-between rounded-lg border border-[#63e6e9]/15 bg-[#63e6e9]/[0.04] px-3 py-2 text-[10px]"><span className="text-[#7187a8]">Detected speech</span><span className="font-semibold text-[#9ee7e9]">{detectSpokenLanguage(transcript).label}</span></div><label className="mb-2 block text-[10px] font-bold uppercase tracking-[0.14em] text-[#748083]">Translate transcript to</label><div className="relative mb-3"><select value={targetLanguage} onChange={(event) => setTargetLanguage(event.target.value)} className="w-full appearance-none rounded-lg border border-white/[0.09] bg-[#1b2644] px-3 py-2.5 text-[11px] text-[#dce6de] outline-none focus:border-[#a78bfa]/60">{languages.filter((item) => item.value !== "auto").map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><ChevronDown size={14} className="pointer-events-none absolute right-3 top-3 text-[#718083]" /></div><div className="grid gap-2 sm:grid-cols-2">{accents.map((accent) => <button key={accent.id} onClick={() => setSelectedAccent(accent.id)} className={`relative rounded-2xl border p-3.5 text-left transition hover:-translate-y-0.5 ${selectedAccent === accent.id ? "border-[#63e6e9]/60 bg-[#63e6e9]/[0.09] shadow-[0_8px_24px_rgba(99,230,233,0.08)]" : "border-white/[0.08] bg-[#111a35]/70 hover:border-white/[0.2]"}`}><div className="flex items-start justify-between gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg" style={{ color: accent.color, backgroundColor: `${accent.color}18` }}><Users size={14} /></span>{selectedAccent === accent.id && <span className="flex h-4 w-4 items-center justify-center rounded-full bg-[#63e6e9] text-[#07171e]"><Check size={10} strokeWidth={3} /></span>}</div><p className="mt-3 text-[11px] font-semibold text-[#dce6de]">{accent.name}</p><p className="mt-1 text-[9px] text-[#718084]">{accent.region}</p><p className="mt-3 font-mono text-[9px] text-[#85918f]">{accent.samples}</p></button>)}</div><div className="mt-5 rounded-2xl border border-[#ff9b9b]/15 bg-[#ff9b9b]/[0.05] p-3.5"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: activeAccent.color }} /><span className="text-[10px] font-semibold text-[#d7dfd6]">Selected: {activeAccent.name}</span></div><p className="mt-1.5 text-[10px] leading-4 text-[#85918b]">This label will be used for per-accent WER and CER reporting.</p></div><button onClick={() => runAnalysis({ transcript, accent: activeAccent.name, language })} disabled={analysisStatus === "running"} className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-[#63e6e9] px-4 py-3 text-[11px] font-bold text-[#07171e] transition hover:bg-[#8ff7f2] disabled:cursor-not-allowed disabled:opacity-40">{analysisStatus === "running" ? <><RotateCcw size={14} className="animate-spin" />Running baseline & fine-tuned models…</> : analysisStatus === "complete" ? <><CheckCircle2 size={14} />Analysis complete — view comparison</> : <><Zap size={14} />Run bias evaluation</>}</button></section>
    </div>
  </>;
}
...
function ClientWorkspace({ session, activeView, goTo, logout, sidebarOpen, setSidebarOpen, transcript, setTranscript }: ClientWorkspaceProps) {
  const navItems = [
...
export default function Index() {
  ...
  return <ClientWorkspace session={session} activeView={activeView} goTo={goTo} logout={logout} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} transcript={transcript} setTranscript={setTranscript} />;
}
