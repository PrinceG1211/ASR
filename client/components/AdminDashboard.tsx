...
import type { ExperimentStage, ExperimentSummary, MlWorkerConfig, MlWorkerConnection, RuntimeCapability, RuntimeStatusResponse } from "@shared/experiment";
...
type AdminDashboardProps = {
  ...
  workerConfig: MlWorkerConfig | null;
  workerConnection: MlWorkerConnection;
  saveWorkerConfig: (config: { provider: string; endpoint: string; model: string; apiKey?: string }) => Promise<void>;
  testWorkerConnection: () => Promise<void>;
  goTo: (view: View) => void;
  logout: () => void;
...
function WorkerConfigSection({ config, connection, saveConfig, testConnection }: { config: MlWorkerConfig | null; connection: MlWorkerConnection; saveConfig: (config: { provider: string; endpoint: string; model: string; apiKey?: string }) => Promise<void>; testConnection: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    setModel(config?.model ?? "openai/whisper-small");
  }, [config]);
  const save = async () => {
    await saveConfig({ provider, endpoint, model, ...(apiKey ? { apiKey } : {}) });
    setApiKey("");
  };
  const statusColor = connection.status === "connected" ? "text-[#56d2aa]" : connection.status === "testing" ? "text-[#e8c76b]" : connection.status === "failed" ? "text-[#f1a0a6]" : "text-[#737d91]";
  return <section className="mb-5 rounded-lg border border-white/[0.06] bg-[#181b23] p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-[#a78bfa]">ML Runtime Configuration</p><h2 className="mt-1 text-[13px] font-bold">Authenticated external Python worker</h2></div><span className={`text-[9px] font-bold uppercase ${statusColor}`}>{connection.status}</span></div><div className="grid gap-3 md:grid-cols-2"><label className="text-[9px] text-[#8b95a8]">ML provider<input value={provider} onChange={(event) => setProvider(event.target.value)} placeholder="Provider name" className="mt-1 w-full rounded-md border border-white/[0.08] bg-[#12141a] px-2.5 py-2 text-[10px] text-[#e9edf6] outline-none" /></label><label className="text-[9px] text-[#8b95a8]">Worker endpoint<input value={endpoint} onChange={(event) => setEndpoint(event.target.value)} placeholder="https://worker.example" className="mt-1 w-full rounded-md border border-white/[0.08] bg-[#12141a] px-2.5 py-2 text-[10px] text-[#e9edf6] outline-none" /></label><label className="text-[9px] text-[#8b95a8]">Model name<input value={model} onChange={(event) => setModel(event.target.value)} placeholder="openai/whisper-small" className="mt-1 w-full rounded-md border border-white/[0.08] bg-[#12141a] px-2.5 py-2 text-[10px] text-[#e9edf6] outline-none" /></label><label className="text-[9px] text-[#8b95a8]">API key / secret<input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={config?.hasApiKey ? "Secret saved on server" : "Enter server-side secret"} autoComplete="new-password" className="mt-1 w-full rounded-md border border-white/[0.08] bg-[#12141a] px-2.5 py-2 text-[10px] text-[#e9edf6] outline-none" /></label></div><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="text-[9px] leading-4 text-[#737d91]">{connection.detail} The secret is never returned to the browser.</p><div className="flex gap-2"><button onClick={() => void save()} className="rounded-md border border-[#8b5bc4]/50 px-3 py-2 text-[9px] font-bold text-[#c4b5fd]">Save configuration</button><button disabled={connection.status === "testing"} onClick={() => void testConnection()} className="rounded-md bg-[#8b5bc4] px-3 py-2 text-[9px] font-bold text-white disabled:opacity-50">Test connection</button></div></div></section>;
}
...
export default function AdminDashboard({ evaluations, experiment, runtimeStatus, checkRuntime, workerConfig, workerConnection, saveWorkerConfig, testWorkerConnection, goTo, logout, runStage }: AdminDashboardProps) {
  ...
  return <div className="min-h-screen bg-[#101217] text-[#e9edf6]"><div className="flex min-h-screen">
    ...
    <main className="min-w-0 flex-1"><header className="flex h-14 items-center justify-between border-b border-white/[0.06] bg-[#13151b] px-5 sm:px-8"><div className="flex items-center gap-3"><button aria-label="Open navigation" onClick={() => setMobileOpen(true)} className="rounded-md p-1.5 text-[#aeb7c9] hover:bg-white/[0.06] lg:hidden"><MenuIcon size={17} /></button><p className="text-[10px] text-[#747d90]">Admin / <span className="text-[#e9edf6]">{currentLabel}</span></p></div><div className="flex items-center gap-3"><span className="hidden text-[9px] text-[#7e8799] sm:block">Experiment state · <span className={experiment?.status === "failed" ? "text-[#f1a0a6]" : "text-[#56d2aa]"}>{stageState}</span></span><div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#6e4ea6] text-[9px] font-bold">AD</div></div></header>
      <div className="mx-auto max-w-[1180px] px-5 py-6 sm:px-8">{settingsOpen && <section className="mb-5 rounded-lg border border-[#8b5bc4]/30 bg-[#1b1825] p-4"><label className="flex items-center gap-2 text-[10px] text-[#c4ccda]"><input type="checkbox" checked={liveUpdates} onChange={(event) => setLiveUpdates(event.target.checked)} className="accent-[#a78bfa]" />Refresh experiment state automatically</label></section>}
        ...
        <WorkerConfigSection config={workerConfig} connection={workerConnection} saveConfig={saveWorkerConfig} testConnection={testWorkerConnection} />
        <section className="mb-5 rounded-lg border border-white/[0.06] bg-[#181b23] p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-[#a78bfa]">ML runtime status</p><h2 className="mt-1 text-[13px] font-bold">Python / Whisper execution readiness</h2></div><div className="flex gap-2"><button onClick={() => void checkRuntime()} className="rounded-md border border-[#8b5bc4]/50 px-3 py-2 text-[9px] font-bold text-[#c4b5fd] hover:bg-[#8b5bc4]/10">Check ML runtime</button></div></div>{runtimeStatus ? <div className="grid gap-x-5 md:grid-cols-2"><RuntimeCapabilityRow label="ML runtime" capability={runtimeStatus.runtime} /><RuntimeCapabilityRow label="Whisper" capability={runtimeStatus.whisper} /><RuntimeCapabilityRow label="Dataset service" capability={runtimeStatus.dataset} /><RuntimeCapabilityRow label="Evaluation service" capability={runtimeStatus.evaluation} /><RuntimeCapabilityRow label="Fine-tuning service" capability={runtimeStatus.fineTuning} /></div> : <p className="text-[10px] text-[#7e8799]">Runtime has not been checked in this session.</p>}</section>
